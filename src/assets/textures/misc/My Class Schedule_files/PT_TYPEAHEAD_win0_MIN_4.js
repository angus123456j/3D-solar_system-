...
PT_typeahead.prototype.TypeAhead = function(xStrText){
 ...
 nsuffix = '$'+j;  name = fname+nsuffix; } else if (ptTAObj_win0.oDoc.getElementById(fname+'$srch'+'$'+j)) {
 nsuffix = '$srch'+'$'+j;  name = fname+nsuffix; } else if (ptTAObj_win0.oDoc.getElementById(fname+'$srch')) {
 nsuffix = '$srch';  name = fname+nsuffix; }
 var el = ptTAObj_win0.oDoc.getElementById(name); if (el && el.getAttribute("focus") == "true") {
 bTAFieldHasFocus = true; return bTAFieldHasFocus; } else {
 var elActiveEl = document.activeElement; if ((el) && (elActiveEl) && (elActiveEl.id == el.id)) {
